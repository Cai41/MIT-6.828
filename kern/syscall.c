...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	if ((r = envid2env(envid, &e, 1)) < 0)
		return r;
	if ((tf->tf_cs & 3) == 0) {
		panic("user env will be running at CPL 0!\n");
	}
	e->env_tf = *tf;
	return 0;
}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	// LAB 4: Your code here.
	struct Env *se = NULL, *de = NULL;
       	int res1 = envid2env(srcenvid, &se, 1);
	if (res1 < 0) return res1;
       	int res2 = envid2env(dstenvid, &de, 1);
	if (res2 < 0) return res2;	

	if (((int)srcva >= UTOP) || ((int)srcva % PGSIZE != 0)) return -E_INVAL;
	if (((int)dstva >= UTOP) || ((int)dstva % PGSIZE != 0)) return -E_INVAL;
	...
	pte_t *entry = NULL;
	struct PageInfo *pp = page_lookup(se->env_pgdir, srcva, &entry);
	...
	res2 = page_insert(de->env_pgdir, pp, dstva, perm);
	return res2;	
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (((uintptr_t)e->env_ipc_dstva < UTOP) && ((uintptr_t)srcva < UTOP)) {
		if (((uintptr_t)e->env_ipc_dstva % PGSIZE != 0) || ((uintptr_t)srcva % PGSIZE != 0))
			return -E_INVAL;
		if (((perm & PTE_U) == 0) || ((perm & PTE_P) == 0) || ((perm | PTE_SYSCALL) != PTE_SYSCALL))
			return -E_INVAL;

		pte_t *entry = NULL;
		struct PageInfo *pp = page_lookup(curenv->env_pgdir, srcva, &entry);
		if (pp == NULL) return -E_INVAL;

...
mem_init(void)
{
	...
	// Your code goes here:
	size_t size_page = ROUNDUP(npages * sizeof(struct PageInfo), PGSIZE);
	boot_map_region(kern_pgdir, UPAGES, size_page, PADDR(pages), PTE_U | PTE_P);
	...
	// LAB 3: Your code here.
	size_t size_env = ROUNDUP(NENV * sizeof(struct Env), PGSIZE);
	boot_map_region(kern_pgdir, UENVS, size_env, PADDR(envs), PTE_U | PTE_P);
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	boot_map_region(kern_pgdir, KERNBASE, (1 << 31) - KERNBASE + (1 << 31), 0, PTE_P | PTE_W);
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_ref != 0 || pp->pp_link != NULL) {
		panic("page_free called with nonzero ref or non NULL link");
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	while (s_walk < (uintptr_t)(va + len)) {
		...
		if (s_walk % PGSIZE == 0) s_walk++;
		s_walk = ROUNDUP(s_walk, PGSIZE);
	}
